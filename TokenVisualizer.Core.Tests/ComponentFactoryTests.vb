...
Namespace TokenVisualizer.Core.Tests
    ...
    Public NotInheritable Class ComponentFactoryTests
        ...
        Public Sub BpeModel_RoundTrips_ByteExact()
            ...
            Dim recon As Object = ComponentFactory.FromModel(JsonNode.Parse(json))
        ...
        Public Sub Normalizers_RoundTrip_ByteExact()
            ...
            For Each n As INormalizer In normalizers
                ...
                Dim recon As INormalizer = ComponentFactory.FromNormalizer(JsonNode.Parse(json))
        ...
        Public Sub Normalizer_BertAndBertNormalizer_Tags()
            Assert.IsInstanceOfType(
                ComponentFactory.FromNormalizer(JsonNode.Parse("{""type"":""Bert"",""clean_text"":true,""handle_chinese_chars"":true,""strip_accents"":null,""lowercase"":true}")),
                GetType(BertNormalizer))
            Assert.IsInstanceOfType(
                ComponentFactory.FromNormalizer(JsonNode.Parse("{""type"":""BertNormalizer"",""clean_text"":true,""handle_chinese_chars"":true,""strip_accents"":null,""lowercase"":true}")),
                GetType(BertNormalizer))
        End Sub
        ...
        Public Sub Normalizer_MissingField_Errors()
            ...
            Assert.ThrowsExactly(Of ArgumentException)(
                Function() ComponentFactory.FromNormalizer(JsonNode.Parse("{""type"":""Sequence"",""prepend_scheme"":""always""}")),
                "missing field `normalizers`")
            ' Empty element cannot be dispatched.
            Assert.ThrowsExactly(Of ArgumentException)(
                Function() ComponentFactory.FromNormalizer(JsonNode.Parse("{""type"":""Sequence"",""normalizers"":[{}]}")),
                "data did not match any variant of untagged enum NormalizerUntagged")
            ' Metaspace-shaped object is not a normalizer.
            Assert.IsNull(ComponentFactory.FromNormalizer(JsonNode.Parse("{""replacement"":""▁"",""prepend_scheme"":""always""}")))
        End Sub
        ...
        Public Sub PreTokenizers_RoundTrip_ByteExact()
            ...
            For Each pt As IPreTokenizer In pretokenizers
                ...
                Dim recon As IPreTokenizer = ComponentFactory.FromPreTokenizer(JsonNode.Parse(json))
        ...
        Public Sub PreTokenizer_LegacyUntagged_Dispatch()
            ...
            Assert.IsInstanceOfType(
                ComponentFactory.FromPreTokenizer(JsonNode.Parse("{""delimiter"":""-""}")),
            ...
            Dim ms As IPreTokenizer = ComponentFactory.FromPreTokenizer(
                JsonNode.Parse("{""replacement"":""▁"",""add_prefix_space"":true}"))
        ...
        Public Sub PreTokenizer_DelimiterVsCharDelimiterSplit_Tags()
            Assert.IsInstanceOfType(
                ComponentFactory.FromPreTokenizer(JsonNode.Parse("{""type"":""Delimiter"",""delimiter"":""-""}")),
                GetType(CharDelimiterSplit))
            Assert.IsInstanceOfType(
                ComponentFactory.FromPreTokenizer(JsonNode.Parse("{""type"":""CharDelimiterSplit"",""delimiter"":""-""}")),
                GetType(CharDelimiterSplit))
        End Sub
        ...
        Public Sub PreTokenizer_MissingField_Errors()
            Assert.ThrowsExactly(Of ArgumentException)(
                Function() ComponentFactory.FromPreTokenizer(JsonNode.Parse("{""type"":""Sequence"",""prepend_scheme"":""always""}")),
                "missing field `pretokenizers`")
            Assert.IsNull(ComponentFactory.FromPreTokenizer(JsonNode.Parse("{""behavior"":""default_split""}")))
        End Sub
        ...
        Public Sub Processors_RoundTrip_ByteExact()
            ...
            For Each pp As IPostProcessor In processors
                ...
                Dim recon As IPostProcessor = ComponentFactory.FromPostProcessor(JsonNode.Parse(json))
        ...
        Public Sub Processor_LegacyUntagged_Dispatch()
            ...
            Dim bert As IPostProcessor = ComponentFactory.FromPostProcessor(
                JsonNode.Parse("{""sep"":[""[SEP]"",102],""cls"":[""[CLS]"",101]}"))
            ...
            Dim roberta As IPostProcessor = ComponentFactory.FromPostProcessor(
                JsonNode.Parse("{""sep"":[""</s>"",2],""cls"":[""<s>"",0],""trim_offsets"":true,""add_prefix_space"":true}"))
        ...
        Public Sub Processor_MissingField_Errors()
            Assert.ThrowsExactly(Of ArgumentException)(
                Function() ComponentFactory.FromPostProcessor(JsonNode.Parse("{""type"":""Sequence"",""prepend_scheme"":""always""}")),
            ...
            Assert.IsInstanceOfType(
                ComponentFactory.FromPostProcessor(
                    JsonNode.Parse("{""add_prefix_space"":true,""trim_offsets"":false,""use_regex"":false}")),
        ...
        Public Sub Decoders_RoundTrip_ByteExact()
            ...
            For Each d As IDecoder In decoders
                ...
                Dim recon As IDecoder = ComponentFactory.FromDecoder(JsonNode.Parse(json))
        ...
        Public Sub Decoder_LegacyUntagged_Dispatch()
            Assert.IsInstanceOfType(
                ComponentFactory.FromDecoder(JsonNode.Parse("{""suffix"":""</w>""}")),
                GetType(BpeDecoder))
            Assert.IsInstanceOfType(
                ComponentFactory.FromDecoder(JsonNode.Parse("{""prefix"":""##"",""cleanup"":true}")),
        ...
        Public Sub Decoder_MissingField_Errors()
            Assert.ThrowsExactly(Of ArgumentException)(
                Function() ComponentFactory.FromDecoder(JsonNode.Parse("{""type"":""Sequence"",""prepend_scheme"":""always""}")),
                "missing field `decoders`")
            ' A Metaspace-shaped object dispatches to MetaspaceDecoder in the legacy path.
            Assert.IsInstanceOfType(
                ComponentFactory.FromDecoder(JsonNode.Parse("{""replacement"":""▁"",""prepend_scheme"":""always""}")),
